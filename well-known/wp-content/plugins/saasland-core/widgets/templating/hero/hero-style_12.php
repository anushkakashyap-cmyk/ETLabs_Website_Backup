<section class="chat_banner_area">
    ...
    if ( !empty( $settings['chat_shape1']['url'] )) { ?>
        <img class="p_absoulte cloud" data-parallax='{"x": 0, "y": 150}' src="<?php echo esc_url( $settings['chat_shape1']['url'] ) ?>">
        <?php
    }
    if ( !empty( $settings['chat_shape2']['url'] )) { ?>
        <img class="p_absoulte left wow fadeInLeft" data-wow-delay="0.2s" src="<?php echo esc_url( $settings['chat_shape2']['url'] ) ?>">
        <?php
    }
    if ( !empty( $settings['chat_shape3']['url'] )) { ?>
        <img class="p_absoulte right wow fadeInRight" data-wow-delay="0.3s" src="<?php echo esc_url( $settings['chat_shape3']['url'] ) ?>">
        <?php
    }
    ?>
    <div class="container">
            ...
            <div class="col-lg-6 <?php echo is_rtl() ? 'offset-lg-2' : ''; ?>">
                <div class="chat_banner_content">
                    ...
                    if ( !empty( $settings['upper_title'] ) ) {
                        echo '<div class="c_tag wow fadeInUp upper-title">' . wp_get_attachment_image( $settings['upper_title_img']['id'], 'full' ) . esc_html( $settings['upper_title'] ) . '</div>';
                    }
                    ...
                    <div class="text-center pr_100 wow fadeInUp" data-wow-delay="0.4s">
                        ...
                        if ( $settings['is_bottom_text_btn'] == 'yes' ) {
                            echo '<span class="button">' . esc_html( $settings['bottom_text_btn'] ) . '</span>';
                        }
                        ?>
                    </div>
                </div>
